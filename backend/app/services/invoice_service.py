...
async def collect_invoice_data(db: Session, from_date: date, to_date: date) -> InvoiceProcess:
    ...
    try:
        ...
        # Only collect shipment data for organisations included in this invoice run
        # and where GetShipmentData is enabled.
        orgs = db.query(Organisation).filter(
            Organisation.org_code.in_(list(current_run_org_codes)),
            Organisation.get_shipment_data == True,
            Organisation.excluded == False
        ).all()
        ...
        for org in orgs:
            ...
            # The final invoice will still use tblShipmentData row count.
            if usage_count != shipment_api_count:
                db.add(InvoiceException(
                    ...
                    severity="Warning",
                    message=(
                        f"OrganizationUsage_Read ShipmentCount is {usage_count}, "
                        f"but Shipments_Read returned {shipment_api_count} records. "
...
def create_final_invoice(db: Session, process_number: int) -> List[FinalInvoiceLine]:
    ...
    # Group usage rows by OrgCode.
    # This prevents duplicate final invoice lines if the Usage API returns more than one row per organisation.
    usage_by_org = {}

    for usage in usages:
        if usage.org_code not in usage_by_org:
            usage_by_org[usage.org_code] = {
                "org_code": usage.org_code,
                "org_full_name": usage.org_full_name,
                "shipment_count": 0
            }

        usage_by_org[usage.org_code]["shipment_count"] += usage.shipment_count or 0

    lines: List[FinalInvoiceLine] = []

    for org_code, usage_data in usage_by_org.items():
        org = db.query(Organisation).filter(Organisation.org_code == org_code).first()
        ...
        plan = db.query(ChargePlan).filter(
            ChargePlan.charge_plan_code == charge_plan_code
        ).first()
        ...
        if org and org.get_shipment_data:
            # For GetShipmentData organisations, ignore tblScmUsage.ShipmentCount.
            # Final invoice must use tblShipmentData rows.
            grouped = defaultdict(int)

            shipments = db.query(ShipmentData).filter(
                ShipmentData.process_number == process_number,
                ShipmentData.org_code == org_code
            ).all()
